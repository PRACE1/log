...
class RetroEffectImpl extends Effect {
  ...
  get colorNum() {
    return (this.uniforms.get('colorNum') as THREE.Uniform).value as number;
  }
  set pixelSize(v: number) {
    (this.uniforms.get('pixelSize') as THREE.Uniform).value = v;
  }
  get pixelSize() {
    return (this.uniforms.get('pixelSize') as THREE.Uniform).value as number;
  }
  constructor() {
...
const RetroEffect = forwardRef(function RetroEffect(
  { colorNum, pixelSize }: { colorNum?: number; pixelSize?: number },
  ref: Ref<unknown>,
) {
...
function DitheredWaves({
  ...
  useFrame(({ clock }) => {
    ...
    if (!prevBackgroundColor.current.every((v, i) => v === backgroundColor[i])) {
      (u.backgroundColor.value as THREE.Color).set(...backgroundColor);
